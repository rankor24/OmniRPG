...
const TabButton: React.FC<{
  ...
  setActiveTab: (scope: CortexScope) => void;
    ...
    <button
        ...
        disabled={disabled}
        className={`flex-shrink-0 sm:flex-1 flex items-center justify-center gap-2 py-3 px-4 text-sm font-semibold border-b-2 transition-all duration-200 whitespace-nowrap ${
            activeTab === scope
...
const MemoryModal: React.FC<MemoryModalProps> = (props) => {
  ...
  const handleSaveMemory = async () => {
    ...
    try {
        await onUpdateMemory(editingItemId, editedContent.trim());
        handleCancelEdit();
    } catch (e) {
  ...
  const handleAdd = async () => {
    if (activeTab === 'style') {
      ...
      if (newItemContent.trim()) {
        ...
        if (memoryScope === 'character') entityId = character?.id;
        else if (memoryScope === 'conversation') entityId = conversationId;
        
        try {
            await onCreateMemory(newItemContent.trim(), memoryScope, entityId);
            setNewItemContent('');
        } catch (e) {
            console.error("Failed to create memory:", e);
            alert("Failed to create memory. " + (e instanceof Error ? e.message : ""));
        }
      }
    }
  };
  ...
  const descriptions: Record<CortexScope, string> = {
    ...
    context: "Select other conversations to provide as additional context. (Full conversation history will be used). Changes are saved instantly.",
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-secondary rounded-lg shadow-xl p-6 w-full max-w-2xl m-4 border border-accent flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex-shrink-0">
            ...
            <div className="flex border-b border-tertiary overflow-x-auto">
                  ...
                  {editingItemId === memory.id ? (
                    ...
                    <>
                      ...
                      <div className="flex-shrink-0 flex items-center">
                         ...
                         <button onClick={() => handleDeleteMemory(memory.id)} className="p-2 text-text-secondary hover:text-danger transition-colors" aria-label="Delete memory"><TrashIcon className="w-5 h-5"/></button>
                      </div>
                    </>
                  )}
                </div>
